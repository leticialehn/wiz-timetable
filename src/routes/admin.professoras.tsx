...
function ProfessorasPage() {
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-6">
      ...
      <form
        onSubmit={(e) => { e.preventDefault(); if (!nome.trim()) return; criar.mutate({ data: { nome, cor } }); setNome(""); }}
      ...
      <ul className="space-y-2">
        {data?.professoras.map((p) => (
          <li key={p.id} className="rounded-lg border border-border p-3 flex items-center gap-3"
            style={{ backgroundColor: p.cor + "40" }}>
            <input
              defaultValue={p.nome}
              onBlur={(e) => e.target.value !== p.nome && atualizar.mutate({ data: { id: p.id, nome: e.target.value, cor: p.cor, ativa: p.ativa } })}
              className="rounded-md border border-input bg-background px-3 py-1.5 text-sm flex-1"
            />
            <input type="color" defaultValue={p.cor}
              onBlur={(e) => e.target.value !== p.cor && atualizar.mutate({ data: { id: p.id, nome: p.nome, cor: e.target.value, ativa: p.ativa } })}
              className="h-9 w-12 rounded border border-input" />
            <label className="text-xs flex items-center gap-1">
              <input type="checkbox" defaultChecked={p.ativa}
                onChange={(e) => atualizar.mutate({ data: { id: p.id, nome: p.nome, cor: p.cor, ativa: e.target.checked } })} />
              Ativa
            </label>
            <button
              onClick={() => confirm(`Remover ${p.nome}?`) && remover.mutate({ data: { id: p.id } })}
